...
for (let i = 0; i < categorias.length; i++) {
    categorias[i].addEventListener("click", (event) => {
        ...
        for (let j = 0; j < productos.length; j++) {
            ...
            let item = "" + productos[j].product_category + "";
            if (item == categoria) {
                console.log("ya")
                addItem(productos[j]);
            ...
            comprar[i].addEventListener("click", (event) => {
                ...
                if(carrito.length!=0){
                    ...
                    for (let j =0; j<carrito.length;j++){
                        repetido = 1;
                        IDC = carrito[j].name;
                        if (IDC == productos[i].product_name) {
                            repetido = 2;
                ...
                if(repetido==1){
                        let producto = `{ 
                            "id": ${productos[i].product_id},
                            "name": "${productos[i].product_name}",
                            "img": "${productos[i].product_img}",
                            "cost": ${productos[i].product_cost},
                            "piezas":${piezas}
                        }`;
